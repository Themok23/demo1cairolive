...
export class PersonProductEntity implements PersonProduct {
  id: string;
  personId: string;
  titleEn: string;
  titleAr?: string | null;
  ...
  constructor(data: PersonProduct) {
    this.id = data.id;
  ...
  update(data: Partial<PersonProduct>): PersonProductEntity {
    return new PersonProductEntity({ ...this, ...data });
  }

  toJSON(): PersonProduct {
    return { ...this };
  }
}
