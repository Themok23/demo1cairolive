...
import { PersonService } from '@/src/domain/entities/personService';
...
export default function PersonServicesTab({ personId }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/admin/people/${personId}/services`)
      .then(r => r.json())
  ...
  async function save() {
    ...
    try {
      const res = await fetch(`/api/admin/people/${personId}/services`, {
        method: 'POST',
  ...
  async function remove(id: string) {
    ...
    await fetch(`/api/admin/people/${personId}/services/${id}`, { method: 'DELETE' });
    setItems((p) => p.filter((i) => i.id !== id));
  }

  async function toggleActive(item: PersonService) {
    await fetch(`/api/admin/people/${personId}/services/${item.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-text-primary">Services ({items.length})</h2>
      ...
      {showForm && (
        <div className="rounded-xl border border-gold/10 bg-surface p-4 space-y-3">
          <div className="grid sm:grid-cols-2 gap-3">
            {([['titleEn','Title (EN) *'],['titleAr','Title (AR)'],['priceText','Price Text'],['externalLink','External Link']] as [keyof Draft, string][]).map(([k, label]) => (
      ...
      <div className="space-y-2">
        ...
        {items.length === 0 && <p className="text-sm text-text-tertiary py-4 text-center">No services yet.</p>}
      </div>
    </div>
  );
}
